...
class Ros(object):   
    def __init__(self): # if needed add publish and subcribe
        try:
            self.pathname = os.environ['HOME'] + "/catkin_ws/src/sock-puppet/"  # /Roni_Ws
            self.data_back = "-5"
        except ValueError:
...
class MoveGroupPythonInterface(Ros):
    """MoveGroup Python Inteface Tutorial
    ...
    def __init__(self):
        ...
        self.robot = moveit_commander.RobotCommander() # This object is the outer-level interface to the robot
        self.scene = moveit_commander.PlanningSceneInterface() # This object is an interface to the world surrounding the robot
        group_name = "manipulator"
        self.move_group = moveit_commander.MoveGroupCommander(group_name) # This object is an interface to one group of joints. This interface can be used to plan and execute motions on the robot
        self.box_name = '' #?
        self.cylinder_name = '' #?
